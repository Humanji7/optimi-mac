...
## ✅ Healthy Projects (${#healthy_projects[@]})
...
## ⚠️ Projects Needing Attention (${#attention_projects[@]})
...
if [[ $with_hook -gt 0 ]]; then
    echo "- **$with_hook active HOOKs** — continue pending work with GUPP" >> "$REPORT_FILE"
fi
...
if [[ -d "$DASHBOARD_DIR" ]]; then
    ...
    if [[ $with_hook -gt 0 ]]; then
        $first || recs_json+=","
        recs_json+="{\"text\":\"$with_hook active HOOK(s) — continue pending work with GUPP\",\"command\":null}"
    fi
    recs_json+="]"
