...
- **commit 5226bad**: fix agent status + animation pause
  - Статус теперь обновляется: IDLE → WORKING при активности терминала
  - WORKING → IDLE через 10 секунд бездействия
  - Uptime обновляется каждые 10 секунд
  - Анимация агента останавливается при выборе (клик)

**Что НЕ проверено (нужно проверить):**
1. Перезапустить `pnpm dev` и проверить:
   - Badge показывает WORKING когда Claude печатает
   - Badge возвращается в IDLE через 10 сек бездействия
   - Анимация паузится при клике на агента
   - Hover tooltip показывает терминал
   - Minimap работает (клик перемещает камеру)
   - Activity timeline показывает события

**Следующие шаги:**
- Проверить багфиксы
- Phase C: Multi-Agent Control (M12-M15) если всё ок
- План в V3_PLAN.md
